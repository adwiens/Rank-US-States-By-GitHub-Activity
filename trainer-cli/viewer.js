...
const all = fs.readFileSync('training-set-labeled.json', 'utf8');
const lines = all.split('\n');
const countsByState = popByState.map(pair => [pair[0], 0]);
for (let line of lines) {
  try {
    const [_rawName, state] = JSON.parse(line);
    if (state != 'X') {
...
console.log(JSON.stringify(_.fromPairs(countsPerCapita), undefined, 2));
